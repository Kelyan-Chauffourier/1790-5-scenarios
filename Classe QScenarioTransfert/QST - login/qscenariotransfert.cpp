...
void QScenarioTransfert::login(QString* user, QString* pass, QString* adress, int port)
{
    lUser = *user;
    lPass = *pass;
    lAdrss = *adress;
...
{
    //récupèrer l'adresse entrer dans login
    QString s_lAdrss = lAdrss;
    s_lAdrss.remove("ftp://" /*0,6*/);
    s_lAdrss.remove("/");
    qDebug() << "valeur de l'adresse";
    qDebug() << s_lAdrss;

    cmds->connectToHost(s_lAdrss, 21);        //"127.0.0.1"
...
void QScenarioTransfert::list(QScenarioDir *directory, QPlainTextEdit& text_edit) //QPlainTextEdit& text_edit
{
    ...
    connect(cmds, SIGNAL(readyRead()), SLOT(readData()));

        cmds->connectToHost("127.0.0.1",21);
        //cmds->write() ;
        if(cmds->waitForConnected() ) {
           cmds->write(data);
        }
    ...
    user_request += lUser;
    user_request += "\r\n";
    QString pass_request;
    pass_request += "PASS ";
    pass_request += lPass;
    pass_request += "\r\n";

    cmds->write(user_request.toStdString().c_str());  //"USER user\r\n"
    cmds->waitForBytesWritten();
    cmds->waitForReadyRead();
    cmds->write(pass_request.toStdString().c_str());  //"PASS 123\r\n"
    cmds->waitForBytesWritten();
    cmds->waitForReadyRead();
        ...
        //liste_lignes[i].remove(0,59) recuperation nom fichier/dossier
    {
        ...
        /*if(cmds->waitForConnected(3000))
          {
                cmds->write("hello server\r\n");
                cmds->waitForBytesWritten(1000);
                cmds->waitForReadyRead(3000);
                qDebug() << "Reading: " << cmds->bytesAvailable();

                qDebug() << cmds->readAll();

                cmds->close();
            }*/
            //data->connectToHost("192.168.20.195",195);
            //readData();
            ...
            cmds->write("LIST\r\n");
            cmds->waitForBytesWritten(1000000);
            cmds->waitForReadyRead(1000000);
            qDebug() << "Reading: " << cmds->bytesAvailable();
            qDebug() << cmds->readAll();*/
...
void QScenarioTransfert::get(QString fileName)
{
 ...
 "C:/Users/franck/Documents/import_server_test/untitled.scn",tr("SCN(*.scn)"));
    ...
    path = new QUrl(lAdrss);        //test sous localhost//"ftp://127.0.0.1/"
    path->setUserName(lUser);       //user
    path->setPassword(lPass);       //123
...
void QScenarioTransfert::put(QString dirPath)
{
    ...
    //addresse à upload
    QUrl uploadurl(lAdrss);
    uploadurl.setUserName(lUser);
    uploadurl.setPassword(lPass);
