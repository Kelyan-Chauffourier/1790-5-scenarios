...
Event::Event(const QList<QString> &args, const QList<QList<QString>> &actions)
: name("unnamed")
, type("manual")
, time(0)
, active(0)
{
    ...
    if(actions.size() > 0) {
        for(int i=0;i<actions.size();i++) {
            this->actions.push_back(Action(actions[i]));
        }
    }
    std::cerr << "EVENT CREATED" << std::endl ;
}

void Event::updateValue(const QString &line, int separator)
{
    QString key ;
    QString value ;
