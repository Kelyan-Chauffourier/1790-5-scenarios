...
void CXmlReader::readData(const QString& xml)
{
    QFile xmlFile(xml) ;
    ...
    while ( !node.isNull() ) {
        ...
        if ( event.tagName() == "event" ) {
            std::cerr << "event detecté" << std::endl ;
